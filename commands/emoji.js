...
const emojiRgb = require('./../util/emojiRgbValues.js');
...
exports.execute = (client, message, args) => {
    ...
    var palette = [];
    for(var i = 0; i < emojiRgb.values.length; i++){
        var obj = {
            R: emojiRgb.values[i].R,
            G: emojiRgb.values[i].G,
            B: emojiRgb.values[i].B,
        }
        palette.push(obj);
    }
    var transColors = [];
            ...
            .write(imgName, () => {
                ...
                Jimp.read(imgName, (err, img) => {
                    ...
                    for(var i = 0; i < transColors.length; i++){
                        ...
                        for(var j = 0; j < emojiRgb.values.length; j++){
                            var e1 = Object.values(emojiRgb.values[j]).slice(0, -1);
                            var e3 = false;
                            if(e1[0] == e2[0] && e1[1] == e2[1] && e1[2] == e2[2]) e3 = true;

                            if(e3){
                                results.push(emojiRgb.values[j].E);
                                break;
                            }
                        }
                    }
